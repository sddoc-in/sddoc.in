...
const NewsLatterBox = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({ message: 'Thanks For Subscribing us', type: 'success' });
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      Your time and privacy are precious. We&apos;ll send only the best, handpicked content. Share your email to start our respectful relationship.  </p>    
      ...
      <div>
        ...
        />
         <p className="dark:text-body-color-dark text-center text-base leading-relaxed text-body-color">
          ...
          {status && (
                <p className={`mt-4 text-base text-center font-medium ${status.type === 'success' ? 'text-green-500' : 'text-red-500'}`}>{status.message}</p>
              )}
        </div>
      </form>
      <div>
